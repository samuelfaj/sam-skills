# Routing Policy (Codex–GLM Flash profile)
...
1. Task classes
2. Certainty budget
3. Capability then provider runtime
4. Fan-out
...
- `T0`: mechanical or read-only, narrow scope, no material runtime/security/data
  risk. Micro: one `LIGHT` GLM-5.3-Flash node max (or controller-only).
- `T1`: one bounded implementation area. Single: one `STANDARD` GLM-5.3-Flash node, or
  `LIGHT` when purely mechanical.
- `T2`: multiple independent slices or cross-file coordination. Multi: min
  independent GLM-5.3-Flash nodes (default parallel 2, hard cap 3) + integration owner.
- `T3`: production, security, auth, privacy, payment, secrets, data loss,
  ...
  GLM-5.3-Flash on the risky slice; serialize unsafe writes; REVIEWER required (Codex
  Sol medium).
...
## Capability then provider runtime

1. Classify `T0`–`T3` and set certainty.
2. Pick mode: Micro / Single / Multi / Critical.
3. Set `task.active_host` to `codex` (controller).
4. Bind the profile matrix row for the capability.
...
### Genius / Sol high (unstick)

Escalate to `genius_worker` (`codex` / `gpt-5.6-sol` / `high`) only after:

| Trigger | Minimum |
| --- | --- |
| `multi_round_fail` | ≥2 GLM-5.3-Flash attempts on same objective with FAIL/capability blocker |
| `stall` | 2× no material progress |
| `deep_insufficient` | Already DEEP GLM-5.3-Flash-max, still unclosed |
| `contradiction` | Claims vs controller proof disagree |

Caps: 2 GLM-5.3-Flash attempts/objective → at most 1 Sol-high → then BLOCKED/user.
Record trigger in `runtime.fallback_reason`. Never escalate because a task is
merely large. Env/CLI failures are blockers, not genius.

`DEEP` is invalid unless classification is `T3` or `risk_flags` is non-empty.

## Review Cost Guard

Use `REVIEWER` only when the review gate triggers. Bind Codex
`gpt-5.6-sol` / `medium`. Independent: different owner; no expected outcome
leaked. Feed **diff + checklist + frozen scope + proof refs** only.
...
Never invent certainty to save cost. Do not raise REVIEWER to Sol high.
...
re-run multi-suite proofs on Codex main thread when required.
