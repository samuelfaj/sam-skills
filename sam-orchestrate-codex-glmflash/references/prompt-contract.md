...
- Effort must match the matrix (`medium` LIGHT, `high` STANDARD, `max` DEEP).
- Invoke `codex exec` with the matrix-bound provider, model, and effort, plus an
  absolute prompt file piped on stdin.
- Never pass `ZAI_API_KEY` or any other secret in the prompt or command line.
