...
Nested agents: pass RC_TOKEN_SAVER_EXECUTION_RECEIPT_V1 and its capability/lane environment unchanged; never put content into it
```

- Pass exact paths, immutable refs, and diff files; never paste file bodies or diffs.
- Never include SKILL.md, references, other workers' reports, secrets (never
  `ZAI_API_KEY`), hidden context, or a reviewer's expected answer.
...
- GLM-5.3-Flash workers: on capability failure, return evidence to the
  controller; never self-escalate to Sol.

## Spawn

Use `<run>` from SKILL.md §1.

- **GLM-5.3-Flash:** the `codex exec` spawn note in host-runtime-matrix.md,
  plus `-o <run>/<node>.last.md` and `> <run>/<node>.log 2>&1`; read only
  `<node>.last.md`.
- **Codex REVIEWER / genius / advisor:** as a Codex agent, or
  `codex exec ... -o <run>/<node>.last.md - < <run>/<node>.prompt.md > <run>/<node>.log 2>&1` reading only
  `<node>.last.md`. The REVIEWER is read-only / ephemeral. These prompts also
  carry the `Nested agents` field.

Read at most the last 60 log lines, only on a non-zero exit. Keep prompt and
log files until the report validates, then delete them; keep the report.
