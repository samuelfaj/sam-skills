# Output Contract (Codex–GLM Flash profile)
...
- Runtime role: `fast_scan`, `routine_worker`, `deep_worker`, `genius_worker`,
  `reviewer`.
- Runtime host: `codex` for all delegated nodes; producer and reviewer models remain distinct.
...
Standard GLM-5.3-Flash producer:
...
```json
{
  ...
  "model": "gpt-5.6-sol",
  "effort": "high",
  "fallback_reason": "stall after 2 GLM-5.3-Flash attempts; evidence V4"
}
```

`fallback_reason` is a non-empty string when the preferred GLM-5.3-Flash row was replaced
...
- All delegated nodes run on `codex`; producer/reviewer separation is enforced by
  distinct models, roles, owners, and read/write boundaries.
...
overlapping writes, proof ownership, review triggers, provider runtime matrix
