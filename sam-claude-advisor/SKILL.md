...
description: "Read-only Claude second opinion on a focused assumption, tradeoff, architecture, security, diagnosis, or high-risk decision. Use when a Claude second opinion or a bounded independent advisory pass is needed."
---
...
- Bind `model` and `effort` in the calling agent: the user's exact override,
  else the `claude-code` advisor row of
...
- Read-only: plan permission mode, only `Read`, `Glob`, `Grep`, no session
  persistence; no edits, shell, subagents, publishing, commits, pushes, or
  external writes.
...
2. Run `python3 -B <abs skill dir>/scripts/resolve_advisor.py --model <model> --effort <effort> --run --prompt-file <abs prompt path>`.
   ...
   the answer; a nonzero exit is the blocker. Without `--run` it prints only
   the argv: run it without a shell, prompt on stdin, and read only the JSON
   `result`.
...
Return `Advisor` (Claude, selected model), `Effort` (value; matrix-default or
