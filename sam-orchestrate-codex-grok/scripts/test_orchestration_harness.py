...
def validate_neutrality_adversaries() -> None:
    ...
    relative_targets = (
        Path("sam-orchestrate-codex-grok/SKILL.md"),
        Path("sam-orchestrate-codex-grok/references/routing-policy.md"),
        Path("sam-orchestrate-codex-grok/agents/openai.yaml"),
    )
...
def valid_genius_escalation() -> dict[str, Any]:
    """STANDARD producer escalated to Codex Sol high after multi-round Grok fail."""
...
def main() -> int:
    ...
    expect_failure(
        ...
        "task.active_host must be codex for the codex-grok profile",
        valid_t0,
    )
