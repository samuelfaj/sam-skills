#!/usr/bin/env python3
"""Validate a deterministic sam-orchestrate-codex-grok execution report."""
...
# Controller is always Codex; producers are Grok by default. Hybrid profile.
CONTROLLER_HOST = "codex"
WORKER_HOSTS = {"codex", "grok"}
RUNTIME_ROLES = {
    "fast_scan",
    "routine_worker",
    "deep_worker",
...
def validate_runtime_binding(
    ...
    *,
    ...
    if host not in WORKER_HOSTS:
        errors.append(f"{label}.runtime.host is invalid for codex-grok profile")
    ...
    if isinstance(fallback, str) and fallback.strip():
        if model not in MATRIX_MODELS:
            errors.append(
                f"{label}.runtime.model is outside the approved codex-grok matrix"
            )
        if effort not in MATRIX_EFFORTS:
            errors.append(
                f"{label}.runtime.effort is outside the approved codex-grok matrix"
            )
        if host not in MATRIX_HOSTS:
            errors.append(
                f"{label}.runtime.host is outside the approved codex-grok matrix"
            )
        return
    errors.append(
...
def validate(report: dict[str, Any]) -> list[str]:
    ...
    if active_host != CONTROLLER_HOST:
        errors.append("task.active_host must be codex for the codex-grok profile")
