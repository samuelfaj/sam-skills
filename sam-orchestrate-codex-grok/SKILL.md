...
description: "Codex-controller orchestration with Grok 4.6 workers (medium/high/xhigh by tier), gpt-5.6-sol medium review, and Sol high only on stall or multi-round escalation. Use when the user runs /sam-orchestrate-codex-grok or wants Codex orchestrating Grok producers for cross-host multi-agent delivery."
---

# Sam Orchestrate Codex–Grok

Controller-only orchestration with a fixed hybrid profile: Codex controls (long
tasks, test re-runs, integration, proof); Grok `grok-4.6` produces; Codex
`gpt-5.6-sol` reviews and unsticks after multi-round Grok failure or stall.
...
- Stay controller-only: delegate production code, tests, docs, migrations, and
  ...
  reporting.
- Profile is fixed: `task.active_host` is `codex`; worker `runtime.host` is
  `grok` or `codex` per §3.
...
- Spawn Grok `EXECUTION` nodes via `sam-grok-worker` with the §3 effort
  (`--effort medium|high|xhigh`), never the worker's default. Spawn the Codex
  REVIEWER and genius via a Codex agent or `codex exec` with the §3 model and
  effort; the REVIEWER is read-only.
- If the Codex or Grok CLI or auth is unavailable, stop with an evidence-backed
...
node fails. The Codex genius is writable only for the frozen scope the
...
effort to `high`; corrections return to producers (Grok, or Sol `high` if a
trigger is already armed). Cap: 3 review rounds (initial + 2), recorded in
