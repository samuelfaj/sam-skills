---
name: sam-claude-opus-advisor
description: "Consult Claude Opus as a read-only advisor for a focused assumption, tradeoff, architecture question, security concern, difficult diagnosis, or high-risk decision. Use when the user requests an Opus second opinion or when another AI agent needs a bounded independent advisory pass; default effort to high unless the user explicitly supplies an effort."
---
...
- Use Claude model alias `opus` exactly. Do not substitute another model.
...
```bash
SAM_CLAUDE_OPUS_ADVISOR_DIR="<absolute directory containing this SKILL.md>"
python3 "$SAM_CLAUDE_OPUS_ADVISOR_DIR/scripts/resolve_advisor.py"
python3 "$SAM_CLAUDE_OPUS_ADVISOR_DIR/scripts/resolve_advisor.py" --effort high
```
...
Opus, the selected effort, plan permission mode, read-only tools, JSON output,
