...
def run_validator(
    ...
    with tempfile.TemporaryDirectory(prefix="sam-orchestrate-clg-test-") as temp_dir:
...
def validate_neutrality_adversaries() -> None:
    ...
    if neutrality_violations(
        {"canonical/references/host-runtime-matrix.md": "model = opus"}
    ):
    ...
    relative_targets = (
        Path("sam-orchestrate-claude-grok/SKILL.md"),
        Path("sam-orchestrate-claude-grok/references/routing-policy.md"),
        Path("sam-orchestrate-claude-grok/agents/openai.yaml"),
    )
    with tempfile.TemporaryDirectory(prefix="sam-orchestrate-clg-neutrality-") as temp_dir:
...
def valid_genius_escalation() -> dict[str, Any]:
    """STANDARD producer escalated to Claude Opus xhigh after multi-round Grok fail."""
        ...
        "dag": [
            node(
                ...
                runtime={
                    ...
                    "model": "opus",
                    "effort": "xhigh",
                    "fallback_reason": "multi_round_fail after 2 grok attempts; evidence prior",
                },
            )
        ],
        "evidence": [evidence("V1", "E1", requirement, evidence_type="DIFF")],
