# Output Contract (Claude–Grok profile)
...
- Active host (controller): **`claude-code` only** for this profile.
...
- Runtime role: `fast_scan`, `routine_worker`, `deep_worker`, `genius_worker`,
  `reviewer`.
- Runtime host: `grok` (LIGHT/STANDARD/DEEP) or `claude-code` (REVIEWER / genius).
...
- `DEEP` capability only when `classification` is `T3` or `risk_flags` is
  non-empty.
- Evidence `detail` should be a short summary (prefer under ~500 characters).

## Runtime binding

`task.active_host` is required and must be `claude-code`. Every delegated
`EXECUTION` and `REVIEW` node requires a `runtime` object matching
...
Advisor (optional, not a production DAG node requirement):
...
- Cross-host is allowed: producers may be `grok` while `active_host` is
  `claude-code` and REVIEWER/genius may be `claude-code`.
...
binding, owner-identity hygiene, and decision consistency.
