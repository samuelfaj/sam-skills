...
{"schema_version": 2,
 "task": {"classification": "T0|T1|T2|T3", "goal": "...", "success_criteria": ["+"], "constraints": [], "no_go": ["+"],
  "risk_flags": [], "active_host": "claude-code", "changed_artifacts": ["<class>"],
 ...
 "dag": [{"id": "E1", "kind": "EXECUTION|ORCHESTRATION|REVIEW", "owner": "worker-N|controller-N|reviewer-N",
  ...
  "runtime": {"host": "grok|claude-code", "role": "fast_scan|routine_worker|deep_worker|genius_worker|ultra_worker|reviewer",
...
- `task`: the report `task` without `changed_*` and `active_host` (always `claude-code`); `constraints`, `risk_flags` default `[]`, `review_requested` false.
