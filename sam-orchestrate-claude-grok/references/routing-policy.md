...
Caps: at most 2 Grok attempts per objective before Opus `xhigh`, then at most 1
Opus `xhigh` attempt, then `BLOCKED` plus a user decision (or the optional
...
may re-run multi-suite proofs on the Claude main thread when required.
